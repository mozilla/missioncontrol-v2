...
ffunc <- function(M,D)  brm(M,data=D, chains = 6, control = list(adapt_delta = 0.999, max_treedepth=12), cores = 6)
...
make.a.model <- function(data,wh,skew=FALSE){
  alter <- TRUE
  if(wh=="cmr"){
    M0 <- bf( log(cmain+1) |weights(cmr.wt*wts)   ~  os+offset(log( usage_cm_crasher_cversion+1/60))  + s(nvc,m=1,by=os) + (1+os|c_version), sigma ~ os+s(nvc,m=1)) 
    if(alter){
  ...
  if(wh=='ccr'){
    M0 <- bf( log(ccontent+1) | weights(ccr.wt*wts)   ~  os+offset(log( usage_cc_crasher_cversion+1/60))  + s(nvc,m=1,by=os) + (1+os|c_version), sigma ~ os+s(nvc,m=1))
    if(alter){
...
getRelDiff <- function(crm.model,crc.model,cim.model,cic.model,current.data,prev.version,CUT=0.35,CUT.I=0.15){
  getDiff <- function(current.data,whi='cmr',model,prev.version,aCUT){
    if(length(whi)==2){
      ## Averages
    ...
    f <- cm.preds[,{
      ...
      h <-  mean(  (this-that)/that > aCUT)
      list(V=if(is.na(h)) 0 else h)
    },by=list(os,nvc)]
    f
  }
  cmr <- getDiff(current.data, whi="cmr", crm.model, prev.version=prev.version,aCUT=CUT)[, cmr.reg.prob := V][,V := NULL]
  ccr <- getDiff(current.data, whi='ccr', crc.model, prev.version=prev.version,aCUT=CUT)[, ccr.reg.prob := V][,V := NULL]
  cavgr <- getDiff(current.data, whi=c('cmr','ccr'), model=list(m=crm.model,c=crc.model),
                   prev.version=prev.version,aCUT=CUT)[, cavgr.reg.prob:=V][,V:=NULL]

  cmi <- getDiff(current.data, whi="cmi", cim.model, prev.version=prev.version,aCUT=CUT.I)[, cmi.reg.prob := V][,V := NULL]
  cci <- getDiff(current.data, whi="cci", cic.model, prev.version=prev.version,aCUT=CUT.I)[, cci.reg.prob := V][,V := NULL]
  cavgi <- getDiff(current.data, whi=c('cmi','cci'), model=list(m=crm.model,c=cim.model),prev.version=prev.version,aCUT=CUT.I)[, cavgi.reg.prob:=V][,V:=NULL]
...
##   pm <- cbind(D[, list(os,c_version, log(usage_cm_crasher_cversion+1/60))],data.table(getPredictions(mdl.rm, D, 'cmr',COUNT=2)))[, {
##     f <- data.table(t(apply(.SD,2,function(x) sum(x[x!=-Inf]))))
##     exp((f[,2:ncol(f), with=FALSE])- (f[,1,with=FALSE][[1]]))
##   },   by=list(os,c_version)][, ":="(c_version = NULL, os=NULL)]
##   print("cmr done")
  
##   pc <- cbind(D[, list(os,c_version, log(usage_cc_crasher_cversion+1/60))],data.table(getPredictions(mdl.rc, D, 'ccr',COUNT=2)))[, {
##     f <- data.table(t(apply(.SD,2,function(x) sum(x[x!=-Inf]))))
##     exp((f[,2:ncol(f), with=FALSE])- (f[,1,with=FALSE][[1]]))
##   },   by=list(os,c_version)][, ":="(c_version = NULL, os=NULL)]
##   print("ccr done")
...
##   pm <- cbind(D[, list(os,c_version, log(dau_cversion))],data.table(getPredictions(mdl.im, D, 'cmi',COUNT=2)))[, {
##     f <- data.table(t(apply(.SD,2,function(x) sum(x[x!=-Inf]))))
##     exp((f[,2:ncol(f), with=FALSE])- f[,1,with=FALSE][[1]])
##   },   by=list(os,c_version)][, ":="(c_version = NULL, os=NULL)]
##   print("cmi done")
  
##   pc <- cbind(D[, list(os,c_version,log(dau_cversion) )],data.table(getPredictions(mdl.ic, D, 'cci',COUNT=2)))[, {
##     f <- data.table(t(apply(.SD,2,function(x) sum(x[x!=-Inf]))))
##     exp((f[,2:ncol(f), with=FALSE])- f[,1,with=FALSE][[1]])
##   },   by=list(os,c_version)][, ":="(c_version = NULL, os=NULL)]
##   print("cci done")
...
beta.summary <- beta.posts[c_version==beta.current.version,][date==max(date),][, os:=factor(os, levels=c("Windows_NT","Darwin","Linux"))]
...
zq <- expression({
  ...
  system(glue("rm -rf  ./visual_demo/older/{today}/", today=(j <- Sys.Date())))
  ## Get Most Recent Model
  as1 <- as.Date(list.files("./visual_demo/older"))
  as1 <- as1[!is.na(as1)] ;
  as2 <- as.character(as1[which.min(max(rel.posts$date) - as1)])
  as3 <- local({
    h <- list(
      rel.posts = fread(glue("./visual_demo/older/{as2}/data/rel.posts.csv",as=as2))[, V1 := NULL][,date := as.Date(date)],
      rel.summary = fread(glue("./visual_demo/older/{as2}/data/rel.summary.csv",as2=as2))[, V1 := NULL][,date := as.Date(date)],
      beta.posts = fread(glue("./visual_demo/older/{as2}/data/beta.posts.csv",as2=as2))[, V1 := NULL][,date := as.Date(date)],
      beta.trend = fread(glue("./visual_demo/older/{as2}/data/beta.trend.csv",as2=as2))[, V1 := NULL],
      beta.summary = fread(glue("./visual_demo/older/{as2}/data/beta.summary.csv",as2=as2))[, V1 := NULL][,date := as.Date(date)]
    )
    h
  })
  ## Whats New
  Fixup <- function(older, newer){
    az1 <- merge(older[, list(use=1),by=list(os,date)],
                 newer[, list(use=1),by=list(os,date)],all.y=TRUE, by=c("os","date"))
    rbind(older,merge(az1[is.na(use.x),list(os,date)], newer))
  }
  rel.posts.1 <- Fixup(as3$rel.posts, rel.posts)
  rel.summary.1 <-rel.summary 
  beta.posts.1 <- Fixup(as3$beta.posts,beta.posts)
...
system(glue("mkdir -p ./visual_demo/older/{today}/", today=j))
