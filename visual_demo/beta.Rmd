...
figs <- list(
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=cavgr.lo90,mn=cavgr.mean,hi90=cavgr.hi90,regProb=cavgr.reg.prob)],
                  "Current Average Crash",""),
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=cmr.lo90,mn=cmr.mean,hi90=cmr.hi90,regProb=cmr.reg.prob)],
                  "Current Browser Crash", ""),
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=ccr.lo90,mn=ccr.mean,hi90=ccr.hi90,regProb=ccr.reg.prob)],
...
```{r}
figs <- list(
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=100*cmi.lo90,mn=100*cmi.mean,hi90=100*cmi.hi90,regProb=cmi.reg.prob)],
                  "Current Browser Crash", ""),
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=100*cci.lo90,mn=100*cci.mean,hi90=100*cci.hi90,regProb=cci.reg.prob)],
                "Current Content Crash",""),
  makeSummaryPlot(beta.summary[, list(fac=as.character(os),lo90=100*cavgi.lo90,mn=100*cavgi.mean,hi90=100*cavgi.hi90,regProb=cavgi.reg.prob)],
                  "Current Average Crash","")
)
grid_plot(figs)
```
...
figs1 <- list(
  ...
  makeAdoptionPlot(beta.posts[os=='Linux' & cmr.wt > 0,
                           list(version=c_version,date=date,adoption=nvc*100,lo90=cmr.lo90,mn=cmr.mean,hi90=cmr.hi90,regProb=100*cmr.reg.prob)]
               , cver=beta.current.version,over=beta.prev.version
...
figs2 <- list(
              ...
              makeAdoptionPlot(beta.posts[os=='Linux',
                           list(version=c_version,date=date,adoption=nvc*100,lo90=100*cmi.lo90,mn=100*cmi.mean,hi90=100*cmi.hi90,regProb=100*cmi.reg.prob)]
               , cver=beta.current.version,over=beta.prev.version
               , xlab='Adoption %', ylab='Linux'
                 )
