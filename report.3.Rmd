...
compare.two <- function(model,whi,this,that,CUT=0.35){
    getDiff <- function(current.data,whi,model){
      ...
      list(plots=plots, means=cm.preds[,mean(x), by=branch] , reldiff=list(i=quantile(reldiff, c(0.5,0.95)),q=quantile(reldiff, c(0.01,0.25,0.5,0.75,0.99)))
         , probReg = mean(reldiff >= (100*CUT))*100,mean = mean(reldiff, trim=0.5/100/2))
    }
...
### Example Usage
#this <- d.beta[c_version=='70.0b4' & os=='Linux',]
#that <- copy(this)[,c_version := '70.0b3']
#compa <- compare.two(model=cr.cc.beta,whi='ccr', this,that,CUT=0.25)
