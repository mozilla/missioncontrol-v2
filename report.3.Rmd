...
make.a.model <- function(data,wh,skew=FALSE){
  if(wh=="cmr"){
    ...
    if(alter){
      M0 <- bf( cmain+1 |weights(cmr.wt)   ~  os+offset(log( usage_cm_crasher_cversion+1/60))  + (1+os|c_version), shape ~ os+s(nvc,m=1))+negbinomial()
    }
  }
  if(wh=='ccr'){
    ...
    if(alter){
      M0 <- bf( ccontent+1 | weights(ccr.wt)   ~  os+offset(log( usage_cc_crasher_cversion+1/60))   + (1+os|c_version), shape ~  os+s(nvc,m=1))+negbinomial()
    }
  }
  if(wh=='cmi'){
...
#alter=TRUE;render("report.3.Rmd");clean_site("./visual_demo");  render_site("./visual_demo/")a
