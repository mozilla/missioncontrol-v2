...
g <- bq()
channel.info <- function(channel,g,nmonth=3){
    list(
        ...
        evolution = posterior.evolution(channel,g=g,nmonth=nmonth),
        change = if(channel %in% c('release','esr')) posterior.version.change(channel,3) else NULL
    )
}
release.info <- channel.info('release',g,nmonth=12)
...
https://chat.mozilla.org/#/room/#stability:mozilla.org (our public Matrix
forum).
