        ...
        layer = list(
            list(
                ...
                encoding = list(
                    ...
                    x = if(x.is.date){
                            list(field="date", timeUnit= "yearmonthdate",type='temporal', axis=list(title="Model Date",labelOverlap='parity')) 
                        }else{
                    ...
                    tooltip=list(list(field = "date",type="temporal"),
...
```{r echo=FALSE}
w <- plotEvolution(release.info$change[modelname=='cr',][, date := as.Date(model_date)], title="Crash Rate SCORE", xtip='Rate',width=990,height=300,x.is.date=TRUE)
vegawidget(as_vegaspec(w),embed=vega_embed(actions=TRUE))

w <- plotEvolution(release.info$change[modelname=='ci',][, ':='(date = as.Date(model_date),c = c*100,lo90=lo90*100,hi90=hi90*100)]
...
```{r echo=FALSE,eval=TRUE}
w <- plotEvolution(esr.info$change[modelname=='cr',][, date := as.Date(model_date)], title="Crash Rate SCORE", xtip='Rate',width=990,height=300,x.is.date=TRUE)
vegawidget(as_vegaspec(w),embed=vega_embed(actions=TRUE))

w <- plotEvolution(esr.info$change[modelname=='ci',][, ':='(date = as.Date(model_date),c = c*100,lo90=lo90*100,hi90=hi90*100)]
                 , title="Crash Incidence SCORE(%)", xtip='Incidence',width=990,height=300,x.is.date=TRUE)
vegawidget(as_vegaspec(w),embed=vega_embed(actions=TRUE))
