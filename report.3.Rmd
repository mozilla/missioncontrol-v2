...
builds <- local({
  ...
  x=rbindlist(Map(function(n,b){
    data.table(versions=n, builds=paste(b,collapse=","))
...
dall.rel2 <- dall.rel2[nvc > 0][,":="(nvc.logit=boot::logit(nvc),cmr.wt = 1*(cmain >0), ccr.wt = 1*(ccontent > 0))]
...
dall.beta2 <- dall.beta2[nvc > 0][,":="(nvc.logit=boot::logit(nvc),cmr.wt = 1*(cmain >0), ccr.wt = 1*(ccontent > 0))]
...
I use a 1% trimmed mean because some of the posterios especially when nvc is
...
rel.current.version <- tail(release.releases.for.model,2)[2,]
rel.prev.version <- tail(release.releases.for.model,2)[1,]
...
beta.current.version <- tail(beta.releases.for.model,2)[2,] 
beta.prev.version <- tail(beta.releases.for.model,2)[1,]
...
system(glue("rsync -avz ./visual_demo/ ./visual_demo/older/{today}/",today=j))
